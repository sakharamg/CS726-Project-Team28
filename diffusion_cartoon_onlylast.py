        ...
        with torch.no_grad():
            ...
            for i in progress_bar(reversed(range(curr_step-n_steps, curr_step)), total=n_steps):
                ...
                if i == 0:
                	...
                	array = x.cpu().numpy()
                	fixed_array=array[0].transpose([1,2,0])
                	#print(fixed_array.shape)
                	# Save the array as a PFM image
                	cv2.imwrite('in.pfm', fixed_array)

                	# Load the PFM image as a NumPy array
                	#array_loaded = cv2.imread('out.pfm', cv2.IMREAD_UNCHANGED)

                	# Convert the NumPy array to a PyTorch tensor
                	#tensor_loaded = torch.from_numpy(array_loaded)
                
                	#print(fixed_array.shape)
                	#print(array_loaded.shape)

                	# Make sure the loaded tensor has the same values as the original tensor
                	#print(torch.all(torch.eq(torch.from_numpy(fixed_array), tensor_loaded)))
                	# Convert the image to PyTorch tensor
                	os.system('python -m image-mcmc --fname in.pfm --it_num 300000 --band 100 --export_fname out.pfm')
                	os.system('python -m image-mcmc --fname in.pfm --it_num 300000 --band 100 --export_fname out.png')
                	array_loaded = cv2.imread('out.pfm', cv2.IMREAD_UNCHANGED)
                	array_loaded_T=array_loaded.transpose([2,0,1])
                	device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                	x = torch.from_numpy (array_loaded_T.reshape ((1,array_loaded_T.shape[0], array_loaded_T.shape[1], array_loaded_T.shape[2]))).to(device)
