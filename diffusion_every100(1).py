...
class func(nn.Module):
    ...
    def forward(self,obj):
        grad=self.grad(obj)
        # Red Increase
        objective=obj[:,0]-0.5*(obj[:,1]+obj[:,2])
        return -torch.sum(objective).to(obj.device)/torch.numel(obj), -grad
    def grad(self,x):
        s=x.shape
        y_=torch.zeros(s).to(x.device)
        y_[:,0,:,:]=torch.ones((s[0],s[2],s[3])).to(x.device)
        y_[:,1,:,:]=-0.5*torch.ones((s[0],s[2],s[3])).to(x.device)
        y_[:,2,:,:]=-0.5*torch.ones((s[0],s[2],s[3])).to(x.device)
        self.grad_= y_/torch.numel(y_)
        return self.grad_
    ...
    def denoise(self, n, n_steps=None, x=None, curr_step=None,
        ...
        with torch.no_grad():
            ...
            if x is None:
                ...
                x = x.to(self.device)
            num_changes=0
            print("initial_score", self.func(x)[0])
            for i in progress_bar(reversed(range(curr_step-n_steps, curr_step)), total=n_steps):
                ...
                # x_copy
                # new_x= custom_diff(x_copy)
                # put new_x in MCMCs
                # Metropolis Adjusted Langevin MCMC -START
#                 if False:
                if i%100==0 and i!=0:
                    ...
                    x_denoised=self.normal_denoise(n,n_steps,x_copy,i)
#                     x_denoised.backwards()
                    out1,grad1=self.func(x_denoised)
                    grad1=grad1
                    x_prime=x-self.tau*grad1 + (2*self.tau)**0.5*eps
                    inp_=x_prime-x+self.tau*grad1


                    tq_xxprime=torch.exp(-0.25/self.tau*torch.norm(inp_)**2)
                    x_prime_copy=x_prime.clone().detach()
                    x_prime_denoised=self.normal_denoise(n,n_steps,x_prime_copy,i)
                    out2,grad2=self.func(x_prime_denoised)
                    inp_2=x-x_prime+self.tau*grad2
                    tq_xprimex=torch.exp(-0.25/self.tau*torch.norm(inp_2)**2)
                    # print(torch.norm(out1-out2))
                    # print(torch.norm(x))
                    # print(torch.norm(x_prime))
                    ratio=-0.25/self.tau*torch.norm(inp_2)**2+0.25/self.tau*torch.norm(inp_)**2
                    # print("ratio: ",ratio)
                    # to_compare=torch.exp(-out2+out1)*tq_xprimex/tq_xxprime
                    to_compare=torch.exp(out2-out1)*torch.exp(ratio)
                    # print(to_compare.shape)
                    A=min(1,to_compare.item())
                    # print("to_comp: ",to_compare.item())
                    u=torch.rand(1).item()
                    if u<A:
                        x=x_prime
                        num_changes+=1
                        print("mcmc update happened")
                    ...
                    for j in range(len(x)):
                        save_image(x, 'x_{}_{}.png'.format(j,i))
                    print("intermediate_score", self.func(x)[0])
                # Metropolis Adjusted Langevin MCMC -END
                callback(x, i, x0=x0)
            print("final_score", self.func(x)[0])
#             print("score: ", out1)
            print("num_changes", num_changes)

            return x

    def normal_denoise(self, n, n_steps=None, x=None, curr_step=None,
                progress_bar=lambda i, total=None: i,
                callback=lambda x, i, x0=None: None):
        with torch.no_grad():
            if curr_step is None:
            ...
            if x is None:
                ...
                x = x.to(self.device)
            num_changes=0
            print("normal denoise initial_score", self.func(x)[0])
            for i in progress_bar(reversed(range(curr_step-n_steps, curr_step)), total=n_steps):
                t = (torch.ones(n)*i).to(self.device)
                x, x0 = denoising_step(x,
                                       t=t,
                ...
                # Metropolis Adjusted Langevin MCMC -START
#                 eps=torch.randn(x.shape).to(x.device)
#                 out1,grad1=self.func(x)
#                 x_prime=x-self.tau*grad1 + (2*self.tau)**0.5*eps
#                 inp_=x_prime-x+self.tau*grad1
                
                
#                 tq_xxprime=torch.exp(-0.25/self.tau*torch.norm(inp_)**2)
#                 out2,grad2=self.func(x_prime)
#                 inp_2=x_prime-x+self.tau*grad2
#                 tq_xprimex=torch.exp(-0.25/self.tau*torch.norm(inp_2)**2)
#                 # print(torch.norm(out1-out2))
#                 # print(torch.norm(x))
#                 # print(torch.norm(x_prime))
