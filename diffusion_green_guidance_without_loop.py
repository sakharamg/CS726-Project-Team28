...
def denoising_step(x, t, *,
    ...
    if looping==1:
        if t<=index_of_change:
            model_output_diff = model(x_ref, t)
            # 2. predict clipped x_0
            # (follows from x_t=sqrt_alpha_cumprod*x_0 + sqrt_one_minus_alpha*eps)
            pred_xstart_diff = (extract(sqrt_recip_alphas_cumprod, t, x.shape)*(x_ref) -
                           extract(sqrt_recipm1_alphas_cumprod, t, x.shape)*model_output_diff)
            pred_xstart_diff = torch.clamp(pred_xstart_diff, -1, 1)
            # 3. compute mean of q(x_{t-1} | x_t, x_0) (eq. (6))
            mean_diff = (extract(posterior_mean_coef1, t, x.shape)*pred_xstart_diff +
                    extract(posterior_mean_coef2, t, x.shape)*(x_ref))
    ...
    if t<=index_of_change:
        if looping==0:
            ...
            #x_old[:,1,:,:]=0
            x_old[:,2,:,:]=0
            model_output_diff = model(x_old, t)
            # 2. predict clipped x_0
            # (follows from x_t=sqrt_alpha_cumprod*x_0 + sqrt_one_minus_alpha*eps)
            pred_xstart_diff = (extract(sqrt_recip_alphas_cumprod, t, x.shape)*(x_old) -
                           extract(sqrt_recipm1_alphas_cumprod, t, x.shape)*model_output_diff)
            pred_xstart_diff = torch.clamp(pred_xstart_diff, -1, 1)
            # 3. compute mean of q(x_{t-1} | x_t, x_0) (eq. (6))
            mean_diff = (extract(posterior_mean_coef1, t, x.shape)*pred_xstart_diff +
                    extract(posterior_mean_coef2, t, x.shape)*(x_old))    
...
#     logvar = extract(logvar, t, x.shape)
    ...
    if t<=index_of_change:
        mean = mean - scale*(mean - mean_diff)  
    else:
        pass
...
if __name__ == "__main__":
    ...
    for ib in tqdm.tqdm(range(nb), desc="Batch"):
        ...
        idx = ib*bs
        diffusion.save(x, f"results/"+name+"/{:06}_green_without_loop.png", start_idx=idx)
