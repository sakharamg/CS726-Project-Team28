...
from torchvision.transforms import GaussianBlur, ToPILImage, ToTensor
...
class func(nn.Module):
    ...
    def forward(self,obj):
        grad=-self.grad(obj)
        objective=obj[:,0]-0.5*(obj[:,2]+obj[:,1]) 
...
def denoised_estimate(x,t_): # Very basic denoiser
    my_min=torch.min(x)
    my_max=torch.max(x)
    blur = GaussianBlur(kernel_size=(5, 5), sigma=(0.1, 2.0))
    x_hat=x.clone().detach() 
    for i in range(int(t_/10)):
        x_hat = blur(x_hat)
    x_hat -= x_hat.min()
    x_hat /= x_hat.max()

    x_hat=x_hat*(my_max-my_min) - my_min
    return x_hat
    ...
    def denoise(self, n, n_steps=None, x=None, curr_step=None,
        ...
        with torch.no_grad():
            ...
            for i in progress_bar(reversed(range(curr_step-n_steps, curr_step)), total=n_steps):
                ...
                t_=(1000-i)
                x_hat=denoised_estimate(x,t_) #Custom function, change this to other

                # Save the blurred image tensor as a PNG file
                # ToPILImage()(blurred_img).save("deblurred_image.png")


                eps=torch.randn(x.shape).to(x.device)
                out1,grad1=self.func(x_hat)
                ...
                x_prime_hat=denoised_estimate(x_prime,t_)
                out2,grad2=self.func(x_prime_hat)
